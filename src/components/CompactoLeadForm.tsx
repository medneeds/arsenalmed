...
export function CompactoLeadForm({ origem = "pagina_compacto" }: { origem?: string }) {
      ...
      <div>
        ...
        <div className="mt-6 border-t border-musgo-300 pt-5">
          <p className="text-[15px] leading-relaxed text-musgo-700">
            O Compacto traz 5 dos 33 cenários. A edição completa entrega os 2 volumes — Manual
            Completo e Catálogo de Fármacos e Tabelas em bônus.
          </p>
          <a
            href="/"
            onClick={() => track("click_buy", { origem: "compacto_form_sucesso" })}
  ...
  return (
    <form onSubmit={handleSubmit}>
      <div className="space-y-5">
        <div>
          <label className={labelClass} htmlFor="cf-nome">
            Nome
          </label>
          <input
            id="cf-nome"
        ...
        <div>
          <label className={labelClass} htmlFor="cf-perfil">
            Você é
          </label>
          <select
            id="cf-perfil"
        ...
        <div className="flex gap-3">
          ...
          />
          <label htmlFor="cf-consent" className="text-[13px] leading-relaxed text-musgo-700">
            {CONSENT_TEXT}{" "}
            <a
              href="/privacidade"
              target="_blank"
      ...
      <p className="mt-3 text-center text-[13px] text-musgo-600">
        Gratuito · Sem cartão · O PDF chega no seu e-mail
      </p>
    </form>
  );
}
